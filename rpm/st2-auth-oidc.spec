...
%post
  %{pip} install --find-links %{st2wheels} --no-index --quiet --upgrade st2-auth-backend-oidc

%postun
  if [ $1 -eq 0 ]; then
    %{pip} uninstall -y --quiet st2-auth-backend-oidc 1>/dev/null || :
  fi
